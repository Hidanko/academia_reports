...
public class App {
	public static void main(String[] args) throws DRException {
		TextColumnBuilder<String> colunaData = col.column("Dia", "dia", type.stringType());
		TextColumnBuilder<Integer> colunaPorcentagem = col.column("Porcentagem", "porcentagem", type.integerType());


		LineChartBuilder lineChart = cht.lineChart()

				.setShowPercentages(true).setShowValues(true)
				.setDataSource(createDatasource())
				.setCategory(colunaData)
				.seriesOrderBy("id").series(cht.serie(colunaPorcentagem));
